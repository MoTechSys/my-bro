...
| ISSUE | الوصف | المالك |
|---|---|---|
| ISSUE-050 | `log_format` لـaccess.log: UC-06 يستخدم `syslog`، UC-09 يتطلّب `apache`؛ يجب توحيد وإعادة اختبار 31168 تحت `apache` | ASTRA (T-12) |
| ISSUE-051 | AR keys [agent,file,md5] قد يرفض محاولات UC-03 المتكررة بنفس المسار؛ يلزم أسماء ملفات فريدة في `eicar_test.sh` + اختبار PILOT | ASTRA (T-11/T-15) |
| ISSUE-052 | دقّة `alert.timestamp` غير مُتحقَّقة (ثانية/ms) — تحدّد دقّة كل مقاييس الزمن | ASTRA (G2 أول فحص) |
| ISSUE-053 | UC-08 `ignore=900` يجعل n=30 = 7.75 ساعة؛ يلزم جدولة متداخلة في `trial_runner.sh` | ASTRA (T-11) |
