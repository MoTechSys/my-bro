...
| T-15 | مراجعة أمنية لكل سكربتات `wazuh/agents/**/active-response` و`scripts/**` (quoting, injection, perms) — **أولوية عالية** بعد ISSUE-036 | ASTRA | 🔒 [ASTRA] 2026-09-09 | تقرير في `tests/SECURITY_REVIEW.md` + fixes | مراجعة جميع ملفات AR وscripts؛ اختبارات محلية ومراجعة Claude وتحقيق معملي موثّقان بشكل منفصل |
