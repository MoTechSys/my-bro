...
[46bbb5ff](https://www.genspark.ai/agents?id=46bbb5ff-865c-58c0-b6bb-9cb17585df6e) مراجعة **آلية ساكنة** للنص المرسل عند5bd2feb. لم يتمكن المراجع من فتحcheckout لديه، ولم يشغّل الاختبارات؛ تقريره يصرح بذلك. الأصل الكامل محفوظ في `research/inbox/2026-09-23_ar_review_result.json` بالمستودع. الحكم التالي أعيد التحقق منه محليًا، وليس تصديقًا تلقائيًا للملاحظات أوتدقيقًا بشريًا:

| البند | الحكم والإجراء | دليل الاختبار/الحد |
|---|---|---|
| F1 اختلافoffset المحاولة عنrun | السلوك قابل للإعادة، لكنه عقدv2 المقصود: تصحيح كل محاولة بـntp_offset_ms الخاص بها، وفحص حدودكلتا المجموعتين. لم نفرض equality صامتة تمنع قياسات ساعة متجددة | اختبار99ms يثبت انقلاب حدالمهلة؛ أضيفclock_correction_source وclock_offsets_differ_from_run. هذه تصريحات تحتاج أدلة ساعة، ولا تحقق أصالة أوتسجيل مسبق؛ لا تضبطoffset من النتيجة |
| F2 تلوث الملكية وفقدالسبب | نرفض تخصيص الدليل المشترك لـB لمجردخطأوقتA؛ لا يثبت ذلك أن الحدث يخصB. فقدالسبب السابق صحيح وقابل للإعادة | يحتفظexclusion_details بخطأtimestamp وجميع أخطاءstage المكتشفة ثمambiguity؛ يبقىالصفانEXCLUDED/AMBIGUOUS. الاختبار الأول للتكرار بنفسfinal كان يحجبخطأtimestamp، فأعيد الإنتاج بمتغيرfinal منفصل |
| F3 UC-07 يقبلVT بدلFIM | صحيح؛ أعيد بـ87105 وكانت النتيجةCOMPLETED_WITHIN_WINDOW | opt-inUC-07 يقبلt2 فقط من100300/100301/100303/100304 بمستوى7؛ اختباراتLinux/Windows تصفIDs لا قبولOS أصلي. بلاpolicy يحتفظv2 بسلوكه السابق |
| F4 stamp يصححNTP | فرضية غير صحيحة: stamp يحولtime zone فقط، ولا يصححoffset. النوافذ والتنبيهات كلاهما raw-manager، ثمcoverage يصححان مرة واحدة | اختبارmanager+90ms قربobserve_until يؤكد الاتساق. لا تغييرلتعريفالنافذة؛ ISO/Z ليس دليلساعةصحيحة |
| F5 مقامالمحفزاتلايشملالمستبعد | صحيح أنهشرطي، وليس عيبًا فيالمقامالشامل؛ كانالاسمبحاجةتوضيح | أضيفobserved_trigger_denominator_scope=non_excluded_trials_with_validated_raw_trigger. اختبارsuccess+blocked يعطي .5 شاملًا و1 شرطيًا معscope صريح |
| F6 سياسةv1 مهملة | صحيح وقابل للإعادة: exit0 بلاAR | load_runs يرفضar_policies فيv1؛ analyze_v2 يجردنسخةlegacy فقط منالسياسة بعدالتحقق ويحفظالأصل للحساب. CLIاختُبرexit2/stdoutفارغ؛ override بالمحاولةمرفوض |
| F7 event_valid غيرموثق | غيرصحيح توثيقيًا: مدرج مسبقًا في«المحاولة والخرج» ويعاداستخدامعقدالربط. لكنالتشخيصv1 مربك | أضيفتشخيصv2 مباشرواختبارغيابالحقل، دونتخفيفشرطالإثباتالمستقل |
| F8 exact keys تكسرالجميععندأيإضافة | غيرصحيح كحكمحالي؛ exact keys اختيارfail-closed، والإضافةالمستقبليةليستتغييرًامنشورًا | لاschema جديدبلاسبب؛ اختبارunknown fields. أيترقيةمستقبليةتحتاجعقد/توافق/اختباراتترحيلصريحة؛ لااستنتاجكسرسجلاتقديمةمنإمكانإضافةجديدة |
| F9 invariant غيرقابلللوصول | حارسدفاعي مقصود؛ كسرهخطأداخليلايجبإخفاؤه كتجربةعادية | أبقيناInputErrorوفشلًاكليًابدلتقريرنجاحجزئي؛ اختبارصفداخليغيرمتسق |

تحكيم ملاحظاتالاختبارات: empty alert set اختبارحقيقي لغيابالمحفز لااختبارلقيمةt2 المصرح بها وحدها. شريحةFIM دونVT تثبت أنغيابمرحلةVT يمنعالمحفز، وتتحقققيودUC-03 أصلًافيvalidate_v2. ترتيبt5 قبل108001 مسموح: وصولالتنبيه النهائي ليس وقتبدءAR ولااكتماله. fixtureUC-03 يجعلVT تنبيهالكشفومحفزAR، وt6 مستقلواختبارهلايملأt5؛ ليستfixture دليلًاعلىسلسلةحذفأصلية. اختبارcompletion_kind يثبتschema فقط، وهذهالحدودمعلنة.

الإصلاح`fb4100a` ثمتحسينfresh-checkout`83ebff9`: **39اختبارAR** (27+12)، و134قياسقديم ناجحة. **680 اختبارًا كليًا في28.865s علىdd87465** وALL CHECKS PASSED. لايحتاجاختبارAR مجلدbuild موجودًا؛ ملفاتCLI مؤقتةتحتجذرالمستودع وتحذفبالسياق. CIالرأسالنهائي منفصل فيPR28.

**تنبيه توافق:**السياسةخاصةبالـrun فيmanifestv2، ولاAPIOverrideبالمحاولة. عدمتساويoffsets ليساعتمادًاللمصدر: clock_ref/time_refs ونسخmanifestالمؤرخة يجب أنتراجعقبلالقياس. تظلالسياسةوالساعاتإقرارات، وليس نظاممصادقةأدلة.
