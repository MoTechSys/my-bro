...
> الحالة: إصلاحات واختبارات محلية؛ **IN-PROGRESS للتحقق الأصلي على Windows والمعمل؛ موافقة Claude مسجلة في #17 §2**. لا اعتماد إنتاجي.
...
هذا تغيير تعاقد توزيع واضح؛ راجعه Claude في #17؛ يبقى منفصلاً عن اعتماد النشر، وعلى الفريق إثبات تشغيل النوع المناسب في المعمل. لا يوسع مرشح VT تلقائياً إلى Windows: `20-virustotal-integration.xml` ما زال يرشح 100200/100201؛ اختبار Windows يحتاج تحقق إعداد تكامله كما في TEST_PLAN.
...
الأوامر القديمة في runbooks قد لا تحمل --lab؛ سيفشل تشغيلها بأمان حتى تُحدّث وفق الجدول. ASTRA تولى الأدلة وحدّث واجهات runbooks والديمو في T-16؛ هذا التقرير هو مرجع واجهات T-15 الجديد.
...
**يبقى T-15 IN-PROGRESS**: المراجعة المستقلة مكتملة #17 وR1/R3 موثقتان في UC-03/07؛ المتبقي معالجة/قبول موثق لمخاطر §4، وبناء Windows وتجارب أصلية على المعمل. يمكن مراجعة وبناء T-11 محلياً أثناء انتظار هذه المتطلبات؛ لا تُشغّل قياسات الحذف على المعمل قبلها. ISSUE-036 حالته «إصلاح أولي في المستودع / مراجَع مستقلاً ويحتاج معملاً»، لا CLOSED.
